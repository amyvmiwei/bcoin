...
Coin.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.spent != null) {
    assert(typeof options.spent === 'boolean',
      'Coinbase must be a boolean.');
...
Coin.prototype.fromReader = function fromReader(data) {
  return this.fromReader(new BufferReader(data));
};
...
Coin.fromReader = function fromReader(data, enc) {
  if (typeof data === 'string')
    data = new Buffer(data, enc);
  return new Coin().fromReader(data);
};
