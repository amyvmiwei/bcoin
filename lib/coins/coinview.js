...
CoinView.prototype.addEntry = function addEntry(outpoint, coin) {
  var coins = this.get(outpoint.hash);

  if (!coins) {
    coins = new Coins();
    coins.hash = outpoint.hash;
    this.add(coins);
  }
  ...
  if (!coins.has(outpoint.index))
    coins.add(outpoint.index, coin);
