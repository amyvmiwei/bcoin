...
CoinView.prototype.addEntry = function addEntry(hash, index, coin) {
...
CoinView.prototype.readCoin = async function readCoin(db, input) {
  ...
  if (!coin) {
    coin = yield db.getRawCoin(prevout.hash, prevout.index);

    if (!coin)
      return;

    return this.addEntry(prevout.hash, prevout.index, coin);
  }
...
CoinView.prototype.fromFast = function fromFast(br, tx) {
  ...
  for (i = 0; i < tx.inputs.length; i++) {
    ...
    coin = RawCoin.fromReader(br);
    this.addEntry(prevout.hash, prevout.index, coin);
  }
