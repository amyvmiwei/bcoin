...
ChainDB.prototype.hasEntry = async function hasEntry(hash) {
  var height = await this.getHeight(hash);
  return height !== -1;
};
...
ChainDB.prototype.getState = async function getState() {
  var data = await this.db.get(layout.R);
...
ChainDB.prototype.getFlags = async function getFlags() {
  var data = await this.db.get(layout.O);
...
ChainDB.prototype.checkDeployments = async function checkDeployments() {
  var raw = await this.db.get(layout.V);
  var invalid = [];
...
ChainDB.prototype.verifyDeployments = async function verifyDeployments() {
  var invalid = await this.checkDeployments();
  var i, bit, batch;
...
ChainDB.prototype.prune = async function prune(tip) {
  ...
  var flags = await this.getFlags();
  var height = await this.getHeight(tip);
...
ChainDB.prototype.getNextHash = async function getNextHash(hash) {
  var data = await this.db.get(layout.n(hash));
...
ChainDB.prototype.getRawCoin = async function getRawCoin(hash, index) {
  ...
  raw = this.coinCache.get(hash + index);

  if (raw)
    return RawCoin.fromRaw(raw);
  ...
  if (state === this.state)
    this.coinCache.set(hash + index, raw);

  return RawCoin.fromRaw(raw);
};

ChainDB.prototype.getCoin = function getCoin(hash, index) {
  var coin = await this.getRawCoin(hash, index);
  return coin.toCoin(hash, index);
};
...
ChainDB.prototype.getCoinView = async function getCoinView(tx) {
  ...
  var i, input, prevout, coin, coins;

  for (i = 0; i < tx.inputs.length; i++) {
    input = tx.inputs[i];
    prevout = input.prevout;
    coin = await this.getRawCoin(prevout.hash, prevout.index);
    ...
    view.addEntry(prevout.hash, prevout.index, coin);
  }
...
ChainDB.prototype.getUndoCoins = async function getUndoCoins(hash) {
  var data = await this.db.get(layout.u(hash));
  if (!data)
...
ChainDB.prototype.getBlock = async function getBlock(hash) {
  var data = await this.getRawBlock(hash);
...
ChainDB.prototype.getBlockView = async function getBlockView(block) {
  var view = new CoinView();
  var undo = await this.getUndoCoins(block.hash());
...
ChainDB.prototype.getTX = async function getTX(hash) {
  var meta = await this.getMeta(hash);
  if (!meta)
    return;
  return meta.tx;
};
...
ChainDB.prototype.getCoinsByAddress = async function getCoinsByAddress(addresses) {
  var coins = [];
  ...
  for (i = 0; i < addresses.length; i++) {
    ...
    for (j = 0; j < keys.length; j++) {
      key = keys[j];
      coin = await this.getCoin(key[0], key[1]);
      assert(coin);
      coins.push(coin);
    }
  }
...
ChainDB.prototype.getHashesByAddress = async function getHashesByAddress(addresses) {
  var hashes = {};
  var i, address, hash;
...
ChainDB.prototype.getTXByAddress = async function getTXByAddress(addresses) {
  var mtxs = await this.getMetaByAddress(addresses);
  var out = [];
  var i, mtx;
...
ChainDB.prototype.getMetaByAddress = async function getTXByAddress(addresses) {
  var txs = [];
  ...
  hashes = await this.getHashesByAddress(addresses);

  for (i = 0; i < hashes.length; i++) {
    hash = hashes[i];
    tx = await this.getMeta(hash);
    assert(tx);
    txs.push(tx);
  }
...
ChainDB.prototype.reset = async function reset(block) {
  var entry = await this.getEntry(block);
  var tip;
...
ChainDB.prototype.removeChains = async function removeChains() {
  ...
  try {
    for (i = 0; i < tips.length; i++)
      await this._removeChain(tips[i]);
  } catch (e) {
    this.drop();
    throw e;
  }
...
ChainDB.prototype._removeChain = async function removeChain(hash) {
  var tip = await this.getEntry(hash);
...
ChainDB.prototype.saveView = function saveView(view) {
  var hashes = view.keys();
  var i, j, hash, coins, coin, raw;

  for (i = 0; i < hashes.length; i++) {
    hash = hashes[i];
    coins = view.get(hash);
    for (j = 0; j < coins.outputs.length; j++) {
      coin = coins.outputs[j];

      if (!coin)
        continue;

      if (coin.spent) {
        this.del(layout.c(hash, j));
        this.coinCache.unpush(hash + j);
        continue;
      }

      raw = coin.toRaw();

      this.put(layout.c(hash, j), raw);
      this.coinCache.push(hash + j, raw);
    }
  }
};
