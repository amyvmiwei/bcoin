...
Chain.prototype.verifyDuplicates = async function verifyDuplicates(block, prev, state) {
  ...
  for (i = 0; i < block.txs.length; i++) {
    tx = block.txs[i];
    result = await this.db.hasCoins(tx.hash());
...
Chain.prototype.verifyInputs = async function verifyInputs(block, prev, state) {
  ...
  for (i = 0; i < block.txs.length; i++) {
    ...
    if (i > 0 && tx.version >= 2) {
      valid = await this.verifyLocks(prev, tx, view, state.lockFlags);
...
Chain.prototype.reorganize = async function reorganize(competitor, block) {
  var tip = this.tip;
  var fork = await this.findFork(tip, competitor);
  var disconnect = [];
  var connect = [];
  var i, entry;
...
Chain.prototype._reset = async function reset(block, silent) {
  var tip = await this.db.reset(block);
  var state;
...
Chain.prototype._replay = async function replay(block, silent) {
  var entry = await this.db.getEntry(block);
...
Chain.prototype.add = async function add(block, flags, id) {
  var hash = block.hash('hex');
  var unlock = await this.locker.lock(hash);
  try {
...
Chain.prototype.isActive = async function isActive(prev, deployment) {
  var state = await this.getState(prev, deployment);
...
Chain.prototype.computeBlockVersion = async function computeBlockVersion(prev) {
  ...
  for (i = 0; i < this.network.deploys.length; i++) {
    ...
    state = await this.getState(prev, deployment);
...
Chain.prototype.getDeploymentState = async function getDeploymentState() {
  var prev = await this.tip.getPrevious();
